...
def bulk_make_draft_payment_entries(payment_requests):
    ...
    frappe.logger().info(f"[Bulk Payment Entry] Starting for: {payment_requests}")
    ...
    for pr_name in payment_requests:
        try:
            ...
            existing_pe = frappe.db.exists(
                "Payment Entry",
                {"reference_no": pr.name, "docstatus": ("<", 2)}
            )
            if existing_pe:
            ...
            if pr.reference_doctype in ["Sales Invoice", "POS Invoice"]:
                ...
                party_account = get_party_account_from_master(
                    party_type, party, pr.company
                )
            ...
            frappe.logger().info(f"""
                ...
                PR: {pr.name}
                Party: {party}
                Selected Bank Account: {bank_account}
            """)
            ...
            if (
                ...
                and party_account_currency != pr.currency
            ):
                exchange_rate = ref_doc.get("conversion_rate") or 1
                bank_amount = flt(
                    pr.outstanding_amount / exchange_rate,
                    pr.precision("grand_total")
                )
            ...
            pe.update({
                ...
                "reference_no": "",
                "reference_date": "",
                "remarks": f"Payment Entry against {pr.reference_doctype} {pr.reference_name} via PR {pr.name}",
            ...
            for ref in pe.references:
                if (
                    ref.reference_doctype == pr.reference_doctype
                    and ref.reference_name == pr.reference_name
                ):
            ...
            frappe.logger().info(f"[SUCCESS] PE Created: {pe.name} for {pr.name}")
            success.append(pe.name)

        except Exception as e:
            error_msg = f"Failed for {pr_name}: {str(e)}"
            frappe.logger().error(error_msg)
            frappe.log_error(frappe.get_traceback(), f"Payment Entry Error: {pr_name}")
